...
def train_all():
    ...
    for e in range(0,epoch):
        ...
        trainer.set_train()
        batch_cnt=0
        for wordVec_en,pos1_en,pos2_en,ep_en,r_en,l_en,wordVec_zh,pos1_zh,pos2_zh,ep_zh,r_zh,l_zh,re,e1,e2 in trainSet.batchs():
            batch_cnt+=1
            ...
            for i in range(D_Times):
                dis_loss=trainer.train_dis(wordVec_en,pos1_en,pos2_en,wordVec_zh,pos1_zh,pos2_zh)
            for i in range(G_Times):
                fool_loss=trainer.fool_dis(wordVec_en,pos1_en,pos2_en,wordVec_zh,pos1_zh,pos2_zh)
            train_loss=trainer.train_RE(wordVec_en,pos1_en,pos2_en,r_en,l_en,wordVec_zh,pos1_zh,pos2_zh,r_zh,l_zh,re_mask)
            print("%s Batch %d: %.3f %.3f %.3f"%(currentTime(),batch_cnt,dis_loss,fool_loss,train_loss))
        test(testSet,"Test",e)
    print("Stop Train")
def test(dataSet,name,Time):
    ...
    stack_label=[]
    stack_pred=[]
    for wordVec_en,pos1_en,pos2_en,ep_en,r_en,l_en,wordVec_zh,pos1_zh,pos2_zh,ep_zh,r_zh,l_zh,re,e1,e2\
        in dataSet.batchs():
        ...
        pred=trainer.eval(wordVec_en,pos1_en,pos2_en,r_en,l_en,wordVec_zh,pos1_zh,pos2_zh,r_zh,l_zh,re_mask)
